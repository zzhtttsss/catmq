...
import org.catmq.util.ContextUtil;
import org.catmq.util.MathUtil;
...
//     */
    ...
    protected class TimedRunnable implements Runnable {
        ...
        TimedRunnable(Runnable runnable) {
            ...
            this.initNanos = MathUtil.nowInNano();
        }

        @Override
        public void run() {
            long startNanos = MathUtil.nowInNano();
            try {
                this.runnable.run();
            } finally {
                long elapsedMicroSec = MathUtil.elapsedMicroSec(startNanos);
    ...
    static class ContextPreservingRunnable implements Runnable {
        ...
        @Override
        public void run() {
            ContextUtil.restoreContext(mdcContextMap);
            try {
                runnable.run();
            } finally {
                MDC.clear();
...
//    }
     ...
     */
    protected OrderedExecutor(String baseName, int numThreads, ThreadFactory threadFactory, boolean traceTaskExecution,
        ...
        checkArgument(!StringUtil.isBlank(baseName));
...
//                ste.registerMetrics(statsLogger);
    ...
    protected static int chooseThreadIdx(long orderingKey, int numThreads) {
        return MathUtil.signSafeMod(orderingKey >>> 1, numThreads);
    }
